...
| 2026-08-23 | all | **Remediation wave — FASE 1 (correctness)**: single routing source (nodes own routing via `Command`; builder no longer registers competing conditional edges — no more double execution); parallel QA fan-out race fixed (partial `stages` updates, `_merge_dict` per-stage merge); stage-ID normalization (`to_stage_id()`, 3 notations) + fail-safe `rollback_to_stage` (unknown target → no-op, never wipes the chain); recovery agent parse fix (no more `locals()` fallback that filled nothing) + category-directed strategy (non-retryable → block without LLM, transient → retry without LLM); `agent_runner` fixes (1 AIMessage per response, real `usage_metadata` in token accounting, hard-timeout via reader thread + queue, opencode output schema-validated, context-budget manager wired from config); **tool sandboxing** (`tools/sandbox.py`: path containment to project root + bash destructive-pattern guard, config `agent.tools.sandbox`) |
