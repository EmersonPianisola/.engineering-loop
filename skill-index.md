...
**Framework:** Engineering Loop v10.5.0
...
| 2026-08-10 | all | v10.5.0 — **Filesystem-driven auto-resume**: sub-agent contract (`sub-agent-contract.md`), state.json as single source of truth, sub-agents write artifacts to disk and return 1 JSON line, orchestrator reads state from disk each iteration, context invariant (~60 lines per iteration), `artifact_path` and `error` fields in state template |
