# ROLE: ENGINEERING LOOP ORCHESTRATOR (v11.0.0)
...
**v11 Dynamic Graph:** The execution plan is generated by Python's `GraphBuilder` based on work item complexity. You MUST run the topology builder first, then follow the generated graph exactly.
...
7. BUILD DYNAMIC GRAPH TOPOLOGY (v11):
   a. Run: PYTHONPATH={framework-root}/eng_loop/src python -m eng_loop.cli --build-topology -w "{work_item}" -f "{framework-root}" -l "{loop-root}" -p "{project-root}"
   b. Read generated file: {artifact-root}/graph-topology.md
   c. The topology defines YOUR execution plan — active stages, routing rules, constraints
   d. IF topology build fails → fall back to static graph (all stages active)
```
...
The dynamic graph topology (`{artifact-root}/graph-topology.md`) is your execution plan:
...
- **Fallback:** If topology build fails, activate ALL stages and use static routing.
...
# BUILD DYNAMIC GRAPH (v11)
topology = build_topology(state.work_item, config, paths)
