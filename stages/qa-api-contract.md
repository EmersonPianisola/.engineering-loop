...
2. Update `{loop-root}/state.json`:
   - `stages.qa.api-contract.done = true` (or `false` on failure)
   - `stages.qa.api-contract.attempts += 1`
   - `stages.qa.api-contract.artifact_path = "artifacts/..."` (your output path)
   - `stages.qa.api-contract.error = null` (or failure description)
3. Record AD-NNN decisions in `{loop-root}/STATE.md ## Decisions` (if applicable)
4. Your response MUST be a single JSON line:
   - Success: `{"stage":"qa.api-contract","status":"done","artifact":"artifacts/..."}`
   - Failure: `{"stage":"qa.api-contract","status":"failed","error":"reason"}`
